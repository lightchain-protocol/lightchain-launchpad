...
import { formatNative, formatNumber, normalizeTokenAddress } from "@/lib/utils";
...
export function TokenDetails() {
  ...
  const marketCapNative = weiToNative(token.marketCap);
  const liquidityNative = weiToNative(token.virtualEthReserve);
  ...
  const statsStripDesktop = (
    <div className="grid grid-cols-2 overflow-hidden rounded-xl border border-border/60 bg-card sm:grid-cols-4">
      <StatItem label="Market Cap" value={formatNative(marketCapNative, nativeSymbol)} />
      <StatItem label="Virtual Liquidity" value={formatNative(liquidityNative, nativeSymbol)} />
      <StatItem label="24H Volume" value={formatNative(volume24hNative, nativeSymbol)} />
  ...
  const statsPillsMobile = (
    <div className="flex flex-wrap gap-2">
      <StatPill label="MC" value={formatNative(marketCapNative, nativeSymbol)} />
      <StatPill label="Liq" value={formatNative(liquidityNative, nativeSymbol)} />
      <StatPill label="Vol" value={formatNative(volume24hNative, nativeSymbol)} />
    </div>
  );
